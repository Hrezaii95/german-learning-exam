...
## ADR-016 — Full Momente A1.1 + A1.2 content rights, conditional on visible attribution

**Status:** Accepted (owner instruction, Claude Code chat, 2026-08-14) — **extends ADR-015**  
**Decision:** The owner holds full rights to use all *Momente* A1.1 and A1.2 material — coursebook, workbook, glossary, transcripts and audio — as product content, **conditional on crediting the book and audio as sources**. The owner states a footnote or a single reference page is sufficient. Claude implements that credit as a persistent, learner-reachable references page listing the exact works used, linked from global navigation, plus per-asset source attribution where media is played.  
**Why:** Authority order puts the owner's latest explicit instruction first. The grant is broader than ADR-015 (which covered Lesson 1–2 audio only) and now also covers A1.2, but it carries an obligation the product must actually discharge.  
**Consequence:** `redistributionBasis` for all Momente-derived media and text is "owner-asserted full rights with attribution, 2026-08-14". **The attribution is a release requirement**: content derived from Momente must not ship without the references page present and reachable. This is enforced by a scripted gate (`tools/audit-attribution.mjs`) rather than by reviewer memory, because an obligation nobody checks is an obligation that silently lapses. The Alpha still ships Lessons 1–2 only — that remains a **scope** decision, not a rights one; A1.2 and later-lesson material are now rights-clear for future scope expansion.  
**Revisit when:** The owner narrows the grant, or a publisher/licence document supersedes the owner's assertion with different attribution wording.

## ADR-017 — Lesson 1–3 vocabulary uses the approved word-family card

**Status:** Accepted by explicit owner request in Codex, 2026-09-03.
**Decision:** Replace website vocabulary study surfaces with the approved `study-guides/card-sample/engineer.html` anatomy. Expand vocabulary coverage to every entry, number and spelling item in `study-guides/lessons-01-03/01-vocabulary.pdf`, including the 48-row teacher professions supplement. Aggregate gender counterparts, plural forms and teacher-listed alternatives. Preserve old detail URLs as aliases of the same family. Keep Core, Context, Classroom and Teacher extra distinctions.
**Execution authority:** The owner's direct instruction authorizes Codex implementation, content integration, testing and the existing push-on-ready delivery for this bounded batch, superseding the older design-only allocation for this work. No human language or listening approval is implied.
**Evidence:** A deterministic catalogue audit must map every PDF entry to exactly one family, verify every supplied form, example, recall answer and source, and check exported routes. Examples authored for the cards are identified in the source drawer. Existing verified media are reused; new generated speech retains candidate listening-review status. No later lesson activities are claimed complete by expanding this vocabulary catalogue.

**Owner clarification, 2026-09-03:** Preserve the previous page, filtering and dashboard UI/UX; change cards only where necessary and push completed sections promptly. Baseline is `54e83a3`, immediately before the card integration. Restore the vocabulary hub's existing search/lesson/category form, Apply/Clear actions and grid, and restore the optional professions collection's existing search, alternatives checkbox and review controls. Feed the complete aggregated inventory into those surfaces. Lesson 3 is an additional value in the same lesson selector. Detail cards retain the approved Learn/Recall format; returning from a card preserves the original filters. The dashboard and shared layout remain byte-identical to the baseline.
