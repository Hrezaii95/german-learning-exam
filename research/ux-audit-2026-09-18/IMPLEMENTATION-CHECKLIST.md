...
**Resume here:** Step 7 → Food. Finish and verify the existing batch, then continue through the unchecked sheet groups. The statuses above reflect the completion records below; they do not require rerunning finished work.

The remaining work, in order:

1. **Finish Food:** add the illustrated menu and ordering exchange using the shared controls. Check article/plural accuracy, audio, saving and recall on phone and desktop; publish and verify live.
2. **Finish Travel, Yesterday and Seasons:** complete each sheet's teaching diagram and practice. Apply the same sheet checkpoint and publish each usable batch.
