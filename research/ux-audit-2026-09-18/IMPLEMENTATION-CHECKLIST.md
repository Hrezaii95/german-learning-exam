...
**Current checkpoint:** All nine steps are complete and published. All 23 audit findings are accounted for. The final live checks passed on 21 September 2026; earlier completion notes are historical evidence, not instructions to repeat work.
...
| 9 | Complete | Verify the release | Live learning journeys, accessibility, data recovery and failure states pass; limitations are recorded. |

**Live release:** [Open German Learning OS](https://hrezaii95.github.io/german-learning-exam/). The implementation checklist is closed; reuse its completed work for any later changes.

**Verification limits:** Physical iPhone/Safari use, a full screen-reader pass and independent native-speaker pronunciation approval remain unverified. Automated checks are not a WCAG certification. Cold first loads took 7–10 seconds in the constrained phone measurement; filtering and search are much faster after loading. These limitations are recorded, not counted as passed checks.
...
- [x] Run required build/content/release checks and the route sweep, then inspect the changed live pages.
- [x] Check keyboard navigation, focus, contrast, text enlargement, touch controls and readable alternatives to visual/drag interactions.
- [x] Check representative slow-network, unavailable-audio, offline, empty-result and failed-import states; measure the large libraries on a constrained phone profile.
- [x] Recheck the approved sheets, backup/restore, filters, pronunciation controls and the complete learning journey after integration. Check German labels, explanation consistency, source attribution and original/generated distinctions; do not count technical playback as pronunciation-quality approval.
...
Step 9 — completed integrated verification and published the resulting fixes: readable contrast/targets, text enlargement and measured navigation clearance; accessible diagram group labels; later-lesson labels; local filtering/search with Back; a smaller listening payload; complete caching of played native recordings; offline book selection; and unbroken phone reader labels. CI passed for final deployment 35546059217 (commit 14eb32fe), including all build/content/publication/security/route gates. The preceding integration release e3606c9e passed all 91 live learner journeys and all 116 rendered views across the original 58-route inventory. The final change only adjusts book-toolbar label wrapping; all seven affected live book/resilience journeys passed again, and live 320/390/1440-pixel checks confirm an intact View label with no horizontal overflow. Original/generated playback, failed-audio recovery, phrase lookup, exact saved return, backup/restore, filters, all fifteen sheets, print summaries and offline recovery have recorded live checks. Final phone rendering was inspected. Local verification covered all 129 distinct browser journeys across the full run and corrected targeted runs; the full check passed 996 tests plus 657 web tests. Automated role warnings were repaired; source-page hotspot size findings retain explicit, verified equivalent Read text controls. Other manual-review scan items remain visible in the raw evidence; no universal accessibility or pronunciation-quality approval is claimed.

Performance checkpoint — isolated live measurements at 390 pixels, 4× CPU slowdown, 1.6 Mbps download and 150 ms latency, with HTTP cache cold and service workers blocked: vocabulary filtering 7,534 → 680 ms; listening filtering 931 → 104 ms; global search 4,953 → 379 ms. Cold load times were 10.3, 7.1 and 9.0 seconds respectively, so initial loading remains a limitation. These are single-run measurements of e3606c9e, not universal speed guarantees; the final book-label-only change is outside the three measured views. Background offline precaching, physical iPhone/Safari, full screen-reader use and independent German listening approval are not certified by these tests.

Final evidence — step-9-final-deployment.json; step-9-live-browser.log (91 passes); step-9-live/summary.json and routes-390.json/routes-1440.json (116 views); step-9-final-live-browser.log (seven final passes); step-9-final-live/reader-labels.json and screenshots; step-9-phone-performance/measurements.json and baseline.json; step-9-live-journeys/artifacts.json (148 retained screenshots/print files); step-9-release-gates.log and step-9-final-check.log. All A01–A23 are mapped to completed checklist steps. Live: https://hrezaii95.github.io/german-learning-exam/.
