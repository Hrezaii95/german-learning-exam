...
try{
  ...
  assert(await page.getByRole('navigation',{name:'Choose a cheat sheet',exact:true}).getByRole('link').count()===11,'All eleven sheets are connected');
  ...
  assert(await page.getByRole('navigation',{name:'Choose a cheat sheet on phone'}).getByRole('link').count()===11,'Phone navigation reaches all eleven sheets');
  await page.locator('.sheet-mobile-switcher summary').click();
