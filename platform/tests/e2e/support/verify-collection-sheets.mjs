...
try{
  for(const [sheet,count] of [['people',119],['verbs',198],['numbers',104],['conversation',192]]){
  ...
  assert(await page.getByRole('button',{name:'Reveal German',exact:true}).count()===198,'Recall mode conceals all German cards');
  await page.getByRole('button',{name:'Reveal German',exact:true}).first().click();
  assert(await page.getByRole('button',{name:'Reveal German',exact:true}).count()===197,'Recall reveals one card at a time');
  ...
  assert(await page.getByRole('navigation',{name:'Choose a cheat sheet on phone'}).getByRole('link').count()===15,'Phone selector exposes all fifteen sheets');
