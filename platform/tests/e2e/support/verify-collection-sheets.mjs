...
try{
  for(const [sheet,count] of [['people',117],['verbs',186],['numbers',104],['conversation',177]]){
  ...
  assert(await page.getByRole('button',{name:'Reveal German',exact:true}).count()===186,'Recall mode conceals all German cards');
  await page.getByRole('button',{name:'Reveal German',exact:true}).first().click();
  assert(await page.getByRole('button',{name:'Reveal German',exact:true}).count()===185,'Recall reveals one card at a time');
  ...
  assert(await page.getByRole('navigation',{name:'Choose a cheat sheet on phone'}).getByRole('link').count()===14,'Phone selector exposes all fourteen sheets');
