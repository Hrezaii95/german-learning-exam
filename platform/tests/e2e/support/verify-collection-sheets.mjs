...
try{
  for(const [sheet,count] of [['people',110],['verbs',147],['numbers',104],['conversation',113]]){
  ...
  assert(await page.getByRole('button',{name:'Reveal German',exact:true}).count()===147,'Recall mode conceals all German cards');
  await page.getByRole('button',{name:'Reveal German',exact:true}).first().click();
  assert(await page.getByRole('button',{name:'Reveal German',exact:true}).count()===146,'Recall reveals one card at a time');
  ...
  assert(await page.getByRole('navigation',{name:'Choose a cheat sheet on phone'}).getByRole('link').count()===10,'Phone selector exposes all ten sheets');
