...
const unit=JSON.parse(await readFile(`apps/web/generated/lesson-${({5:'five',6:'six',7:'seven',8:'eight'})[lesson]}-study.json`,'utf8'));
...
try{
 ...
 await tabs.getByRole('button',{name:'Words',exact:true}).click();check(await page.locator('[data-word-family]').count()===({5:98,6:73,7:93,8:74})[lesson],'Every source entry has a word card');
 ...
 await visit(lesson===5?'/cheat-sheets/objects/':lesson===6?'/cheat-sheets/office/':lesson===7?'/cheat-sheets/hobbies/':'/cheat-sheets/time/');await page.screenshot({path:resolve(output,'sheet-mobile.png')});check(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'Cheat sheet fits phone width');
