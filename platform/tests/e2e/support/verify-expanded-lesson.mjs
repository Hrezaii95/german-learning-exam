...
const unit=JSON.parse(await readFile(`apps/web/generated/lesson-${({5:'five',6:'six',7:'seven',8:'eight',9:'nine',10:'ten',11:'eleven',12:'twelve'})[lesson]}-study.json`,'utf8'));
...
try{
 ...
 await tabs.getByRole('button',{name:'Words',exact:true}).click();check(await page.locator('[data-word-family]').count()===({5:98,6:73,7:93,8:74,9:100,10:86,11:81,12:102})[lesson],'Every source entry has a word card');
 ...
 await visit(lesson===5?'/cheat-sheets/objects/':lesson===6?'/cheat-sheets/office/':lesson===7?'/cheat-sheets/hobbies/':lesson===8?'/cheat-sheets/time/':lesson===9?'/cheat-sheets/food/':lesson===10?'/cheat-sheets/travel/':lesson===11?'/cheat-sheets/past/':'/cheat-sheets/journeys/');await page.screenshot({path:resolve(output,'sheet-mobile.png')});check(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'Cheat sheet fits phone width');
