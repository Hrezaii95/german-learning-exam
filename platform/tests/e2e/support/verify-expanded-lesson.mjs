...
const unit=JSON.parse(await readFile(`apps/web/generated/lesson-${({5:'five',6:'six',7:'seven',8:'eight',9:'nine'})[lesson]}-study.json`,'utf8'));
...
try{
 ...
 await tabs.getByRole('button',{name:'Words',exact:true}).click();check(await page.locator('[data-word-family]').count()===({5:98,6:73,7:93,8:74,9:100})[lesson],'Every source entry has a word card');
