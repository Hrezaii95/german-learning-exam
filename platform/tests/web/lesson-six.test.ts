...
describe("Lesson 6 and Module 2 integration",()=>{
 ...
 it("keeps official keys and model answers distinct and exposes irregular verbs",()=>{
  const answers=loadBookAnswers();expect(answers).toHaveLength(146);expect(answers.filter(a=>a.pageId==='workbook-40'&&a.kind==='sample')).toHaveLength(1);expect(answers.find(a=>a.pageId==='workbook-43')?.text).toContain('15 rote Notizbücher');
