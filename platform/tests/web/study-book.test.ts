...
describe("Lesson 4 source and exercise integration", () => {
  ...
  it("has all four original coursebook and workbook pages for each lesson", () => {
    expect(book.pages).toHaveLength(65);
    for (const lesson of [1, 2, 3, 4]) for (const kind of ["coursebook", "workbook"]) expect(book.pages.filter(p => p.lesson === lesson && p.kind === kind && p.section===`Lesson ${lesson}`)).toHaveLength(4);
