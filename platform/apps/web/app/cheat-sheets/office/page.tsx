...
export default function Page(){return <ShellLayout current="cheat-sheets"><OfficeCheatSheet alphabet={loadWordCards().cards.filter(card=>card.category==="Alphabet")} cards={loadWordCards().cards.filter(c=>c.studyTags?.lessons.includes(6))} speech={{...loadCollectionSpeech(),...loadStudySpeech()}}/></ShellLayout>;}
