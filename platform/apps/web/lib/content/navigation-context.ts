...
export type NavigationContext = {
  ...
  hubId?: LearnerHubId;
  /** Hub lesson filter. */
  lesson?: "all" | "01" | "02" | "03" | "04" | "05" | "06" | "07";
  /** Hub category filter. */
  category?: string;
...
export function isSafeNavigationPath(pathname: string): boolean {
  ...
  const lessonMatch = pathname.match(/^\/lessons\/(01|02|03|04|05|06|07)$/);
...
function normalizeContext(raw: unknown): NavigationContext | null {
  ...
  if (entryContext === "hub") {
    ...
    if (obj.lesson === "01" || obj.lesson === "02" || obj.lesson === "03" || obj.lesson === "04" || obj.lesson === "05" || obj.lesson === "06" || obj.lesson === "07" || obj.lesson === "all") {
...
export function buildHubNavigationContext(input: {
  ...
  if (q.length > 0) ctx.q = q;
  if (input.lesson === "01" || input.lesson === "02" || input.lesson === "03" || input.lesson === "04" || input.lesson === "05" || input.lesson === "06" || input.lesson === "07") ctx.lesson = input.lesson;
  if (input.category) {
