...
export function VerbWorkshop({cards,speech}:{cards:WordCard[];speech:Record<string,string>}){
  ...
  return <>
    <section className="sheet-workshop" id="verb-explorer"><p className="study-eyebrow">Person → stem → ending</p><h2>See what changes. Hear the whole form.</h2>
      ...
      <div role="group" className="verb-anatomy" aria-label="Published verb form, broken into parts"><div><small>Person</small><strong lang="de">{verbPersons[person]}</strong></div><span aria-hidden="true">+</span><div className="verb-stem"><small>{anatomy.whole?"Whole exception":anatomy.changed?"Changed stem":"Stem"}</small><strong lang="de">{anatomy.stem}</strong></div>{!anatomy.whole&&<><span aria-hidden="true">+</span><div className="verb-ending"><small>Ending</small><strong lang="de">{anatomy.ending||"∅"}</strong>{!anatomy.ending&&<span>No added ending</span>}</div></>}{anatomy.rest&&<div className="verb-tail"><small>{anatomy.restLabel}</small><strong lang="de">{anatomy.rest}</strong></div>}</div>
