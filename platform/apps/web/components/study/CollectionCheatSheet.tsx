...
export function CollectionCheatSheet({sheet,cards,speech}:{sheet:ExtendedSheetId;cards:WordCard[];speech:Record<string,string>}){
  ...
  const scopedQuestions=sheet==="people"?peopleRecallQuestions(visible):sheet==="verbs"?verbRecallQuestions(visible):sheet==="numbers"?numberRecallQuestions(visible):[];
  const hasScopedPractice=sheet==="people"||sheet==="verbs"||sheet==="numbers";
  ...
  return <div className={`collection-sheet collection-${sheet}`}>
    ...
    <div id="sheet-workshop">{sheet==="numbers"?<NumbersWorkshop cards={visible} allCards={cards} speech={speech}/>:<SheetWorkshops sheet={sheet} cards={sheet==="verbs"?cards:scopedCards} speech={speech}/>}</div>
    ...
    {sheet==="people"&&<PeoplePrintSummary/>}{sheet==="verbs"&&<VerbPrintSummary/>}{sheet==="numbers"&&<NumbersPrintSummary/>}
  </div>;
}
