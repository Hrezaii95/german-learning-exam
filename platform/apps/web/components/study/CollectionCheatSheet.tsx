...
export function CollectionCheatSheet({sheet,cards,speech}:{sheet:ExtendedSheetId;cards:WordCard[];speech:Record<string,string>}){
  ...
  const peopleQuestions=sheet==="people"?peopleRecallQuestions(visible):[];
  ...
  return <div className={`collection-sheet collection-${sheet}`}>
    ...
    <div id="sheet-workshop"><SheetWorkshops sheet={sheet} cards={scopedCards} speech={speech}/></div>
    ...
    {sheet==="people"&&<PeoplePrintSummary/>}
  </div>;
}
